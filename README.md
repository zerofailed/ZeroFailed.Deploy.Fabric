...
| `-TimeoutSeconds` | `int` | How long to wait for provisioning, for the connection to appear, and for an approval to take effect. Default: 300. Applied per endpoint |
...
> **The timeout is per endpoint.** When a connection can't be matched on the target resource, each endpoint waits the full `-TimeoutSeconds` before being reported as `NotFound`, so a wrong name pattern is slow as well as wrong. The default is 300 seconds; lower it (or `$FabricManagedPrivateEndpointApprovalTimeoutSeconds`) while checking that the pattern matches, and raise it if endpoints are slow to provision.
